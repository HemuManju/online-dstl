...
class Maxish(torch.nn.Module):
    ...
    def distributed_true_max(xx, dim=1, keepdim=True):
        ...
        inds = xx == m
        return torch.where(inds, xx, xx * 0).sum(dim, keepdim=keepdim) / inds.sum(
            dim, keepdim=keepdim
        )
        ...
        if scale > 0:
            if agm == True:
                if torch.gt(x, 0).all():
                    ...
                    return (torch.lt(x, 0) * x).sum(dim, keepdim=keepdim) / torch.lt(
                        x, 0
    ...
    def forward(
        ...
        **kwargs
        ...
        if isinstance(inputs, Expression):
            ...
            _inputs = inputs.value
        elif isinstance(inputs, torch.Tensor):
            _inputs = inputs
        elif isinstance(inputs, tuple):
            _inputs = convert_to_input_values(inputs)
        else:
            raise ValueError("Not a invalid input trace")

        robustness_trace = formula.robustness_trace(
            _inputs,
            pscale=pscale,
            scale=scale,
            keepdim=keepdim,
...
class Temporal_Operator(STL_Formula):
    ...
    def __init__(self, subformula, interval=None, robustness_interval=None):
        super(Temporal_Operator, self).__init__()
        self.subformula = subformula
        self.interval = interval

        if robustness_interval is None:
            self.robustness_interval = torch.tensor(0.0)
        else:
            self.robustness_interval = torch.tensor(robustness_interval)

        self._interval = [0, np.inf] if self.interval is None else self.interval
        self.rnn_dim = (
            1 if not self.interval else self.interval[-1]
        )  # rnn_dim=1 if interval is [0, ∞) otherwise rnn_dim=end of interval
        if self.rnn_dim == np.inf:
            self.rnn_dim = self.interval[0]
        self.steps = (
            1 if not self.interval else self.interval[-1] - self.interval[0] + 1
        )  # steps=1 if interval is [0, ∞) otherwise steps=length of interval
        self.operation = None
    ...
    def _run_cell(self, x, scale, time_steps=None, agm=False, distributed=False):
        '''
        ...
        for i in range(time_dim):
            if time_dim < self.get_horizon():
                o = xs[-1].detach()*0
            else:
                o, hc = self._rnn_cell(
                    xs[i], hc, scale, agm=agm, distributed=distributed
                )
            outputs.append(o)
            states.append(hc)
    ...
    def robustness_trace(
        ...
        **kwargs
        ...
        outputs, states = self._run_cell(
            trace, time_steps=time_steps, scale=scale, agm=agm, distributed=distributed
        )
    ...
    def get_horizon(self):
        return self.subformula.get_horizon()
...
class Always(Temporal_Operator):
        ...
        if self.interval is None:
            if distributed:
                ...
                output = self.operation(
                    input_, scale, dim=1, keepdim=True, agm=agm
                )  # [batch_size, 1, x_dim]
                state = (output, None)
            ...
            if (self._interval[1] == np.inf) & (self._interval[0] > 0):
                ...
                output = self.operation(
                    dh, scale, dim=1, keepdim=True, agm=agm, distributed=distributed
                )  # [batch_size, 1, x_dim]
                ...
                output = self.operation(
                    input_, scale, dim=1, keepdim=True, agm=agm, distributed=distributed
                )  # [batch_size, 1, x_dim]
        return output, state
    ...
    def get_horizon(self):
        if self._interval is None:
            ...
            return torch.tensor(self._interval[1])
...
class Eventually(Temporal_Operator):
    def __init__(
        self, subformula='Eventually input', interval=None, robustness_interval=None
    ):
...
class Negation(STL_Formula):
    ...
    def robustness_trace(
        self, inputs, pscale=1, scale=-1, keepdim=True, distributed=False, **kwargs
    ):
        return -self.subformula(
            inputs,
            pscale=pscale,
    ...
    def robustness_trace(
        ...
        **kwargs
        ...
        return self.operation(
            xx, scale, dim=-1, keepdim=False, agm=agm, distributed=distributed
        )  # [batch_size, time_dim, ...]

    def _next_function(self):
        # next function is actually input (traverses the graph backwards)
        return torch.maximum(
            self.subformula1.get_horizon(), self.subformula2.get_horizon()
        )
...
class Until(STL_Formula):
    def __init__(
        self,
        subformula1="Until subformula1",
        subformula2="Until subformula2",
        interval=None,
        overlap=True,
    ):
        '''
    ...
    def robustness_trace(
        ...
        **kwargs
        ...
        '''
        assert isinstance(
            self.subformula1, STL_Formula
        ), "Subformula1 needs to be an stl formula"
        assert isinstance(
            self.subformula2, STL_Formula
        ...
        maxish = Maxish()
        LHS = (
            trace2.unsqueeze(-1).repeat([1, 1, 1, trace2.shape[1]]).permute(0, 3, 2, 1)
        )
        if interval == None:
    ...
    def get_horizon(self):
        return self.interval[1] + torch.maximum(
            self.subformula1.get_horizon(), self.subformula2.get_horizon()
        )
...
class Integral1d(STL_Formula):
    ...
    def robustness_trace(
        ...
        **kwargs
        ...
        subformula_trace = self.subformula(
            inputs, pscale=pscale, scale=scale, keepdim=keepdim, **kwargs
        )
        ...
        if self.interval is not None:
            ...
            if use_relu:
                return self.conv(torch.relu(signal)).transpose(1, 2)[
                    :, : subformula_trace.shape[1], :
                ]
            else:
                return self.conv(signal).transpose(1, 2)[
                    :, : subformula_trace.shape[1], :
                ]
        else:
            if integration_scheme == "trapz":
                ...
                signal = torch.cat(
                    [
                        pad,
                        (subformula_trace[:, :-1, :] + subformula_trace[:, 1:, :]) / 2,
                    ],
                    dim=1,
                )
            else:
